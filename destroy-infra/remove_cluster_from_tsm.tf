...
data "http" "delete_cluster_from_tsm" {
  provider = http-full
...
output "delete_cluster_id" {
  depends_on = [
    data.http.exchange_token_remove,
    data.http.delete_cluster_from_tsm
  ]
  value       = jsondecode(data.http.delete_cluster_from_tsm.response_body)["id"]
  sensitive   = true
  description = "Removed cluster from TSM"
}